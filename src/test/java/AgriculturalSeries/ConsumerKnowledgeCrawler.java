...
/**
 * 消费常识爬虫 http://www.gdagri.gov.cn/fwpt/scfw/xfcs/（目前仅一页，需加注意！）
 * Created by Bin on 2015/3/25.
 */
public class ConsumerKnowledgeCrawler extends BaseCrawler {
    RegexRule regexRule;

    public ConsumerKnowledgeCrawler() {
        ...
        String seedUrl;
        for (int i = 0; i < 1; i++) {
            seedUrl = "http://www.gdagri.gov.cn/fwpt/scfw/xfcs/";
            if (i != 0) {
    ...
    public Links customizeAndGetLinks(FetchQueueItem fetchQueueItem, Document document) {
        ...
        if (fetchQueueItem.getLayer() == 1) {
            ...
            gdagriCrawlerData.setMemo("消费常识");
