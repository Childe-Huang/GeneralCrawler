...
/**
 * 工作动态爬虫 http://www.gdagri.gov.cn/zwgk/gzdt/
 * Created by Bin on 2015/3/25.
 */
public class DynamicWorkCrawler extends BaseCrawler{
    RegexRule regexRule;

    public DynamicWorkCrawler() {
        ...
        String seedUrl;
        for (int i = 0; i < 3; i++) {
            seedUrl = "http://www.gdagri.gov.cn/zwgk/gzdt/";
            if (i != 0) {
    ...
    public Links customizeAndGetLinks(FetchQueueItem fetchQueueItem, Document document) {
        ...
        if (fetchQueueItem.getLayer() == 1) {
            ...
            gdagriCrawlerData.setMemo("工作动态");
