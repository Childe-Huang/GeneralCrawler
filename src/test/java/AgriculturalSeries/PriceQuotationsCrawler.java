...
/**
 * 价格行情爬虫 http://www.gdagri.gov.cn/fwpt/scfw/jgxq/
 * Created by Bin on 2015/3/25.
 */
public class PriceQuotationsCrawler extends BaseCrawler {
    RegexRule regexRule;

    public PriceQuotationsCrawler() {
        ...
        String seedUrl;
        for (int i = 0; i < 3; i++) {
            seedUrl = "http://www.gdagri.gov.cn/fwpt/scfw/jgxq/";
            if (i != 0) {
    ...
    public Links customizeAndGetLinks(FetchQueueItem fetchQueueItem, Document document) {
        ...
        if (fetchQueueItem.getLayer() == 1) {
            ...
            gdagriCrawlerData.setMemo("价格行情");
